defmodule VzBeam.ProtocolTest do
  ...
  test "collect: error event dominates even with a terminal present" do
    lines = [
      ~s({"type":"restored","build":"X"}),
      ~s({"type":"error","domain":"VZ","code":6,"message":"boom"})
    ]
  ...
  test "collect: no terminal -> :no_terminal; unterminated final line -> :unterminated" do
    assert {:error, :no_terminal} =
             Protocol.collect([~s({"type":"progress"})], ["restored"], true)

    assert {:error, :unterminated} =
             Protocol.collect([~s({"type":"restored"})], ["restored"], false)
  end
