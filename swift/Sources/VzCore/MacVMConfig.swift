...
func buildMacConfiguration(_ o: RunOpts) throws -> VZVirtualMachineConfiguration {
    guard let hardwareModel = o.hardwareModel,
          let hwData = Data(base64Encoded: hardwareModel),
          let hw = VZMacHardwareModel(dataRepresentation: hwData) else {
        throw ConfigError.badField("hardware-model")
    }
    guard let idData = Data(base64Encoded: o.machineId),
          let mid = VZMacMachineIdentifier(dataRepresentation: idData) else {
        throw ConfigError.badField("machine-id")
    }
    guard let mac = VZMACAddress(string: o.mac) else { throw ConfigError.badField("mac") }
    guard let aux = o.aux else { throw ConfigError.badField("aux") }

    let platform = VZMacPlatformConfiguration()
    platform.hardwareModel = hw
    platform.machineIdentifier = mid
    platform.auxiliaryStorage = VZMacAuxiliaryStorage(url: URL(fileURLWithPath: aux))
