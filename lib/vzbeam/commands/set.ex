defmodule VzBeam.Commands.Set do
  ...
  alias VzBeam.{Manifest, Pidfile, Home, Disk, GuestPolicy}
  ...
  defp apply_set(name, opts) do
    with :ok <- validate(opts),
      ...
      {:ok, [summary(name, updated) | disk_hint(updated, opts[:disk_gb])]}
    else
      err -> error(name, err)
    end
  end
  ...
  defp summary(name, m) do
    [
      ...
      " mem=",
      to_string(div(m["memoryBytes"], @gb)),
      "G",
      " disk=",
      Disk.gb(Disk.size(disk_path(name))),
      "\n"
    ]
  end

  defp disk_hint(_manifest, nil), do: []

  defp disk_hint(manifest, _gb) do
    disk_hint(GuestPolicy.disk_growth_note(manifest))
  end

  defp disk_hint(:macos_recovery_partition) do
    [
  ...
  defp disk_hint(:openbsd_unallocated_space) do
    [
      "note: the host disk image grew, leaving unallocated guest space. Use OpenBSD disk and filesystem tools\n",
      "inside the guest to partition and grow into that space.\n"
    ]
  end
  ...
  defp error(name, {:error, :running}),
    do: {:error, 1, ["set: ", name, " is running; stop it first\n"]}
  ...
  defp error(_n, {:error, :invalid_manifest}), do: manifest_error(:invalid_manifest)
  defp error(_n, {:error, {:unsupported_schema, _} = reason}), do: manifest_error(reason)
  defp error(_n, {:error, {:unsupported_guest, _} = reason}), do: manifest_error(reason)
  ...
  defp manifest_error(reason), do: {:error, 1, ["set: ", Manifest.describe_error(reason), "\n"]}
end
